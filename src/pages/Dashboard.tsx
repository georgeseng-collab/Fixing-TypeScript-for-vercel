...
export default function Dashboard() {
  ...
  const handleBooking = (app, type) => {
    ...
    // FORCE SINGAPORE OFFSET IN STRING
    ...
    // Explicitly add &ctz=Asia/Singapore
  ...
  if (loading) return <div className="h-screen flex items-center justify-center font-black text-slate-300 text-3xl italic animate-pulse">FIXING TIMESTAMPS...</div>;
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
        {filtered.map(app => (
          <div key={app.id} className="bg-white rounded-[3rem] border border-slate-100 shadow-xl overflow-hidden relative">
            ...
            {/* HISTORY OVERLAY - FORCED FORMATTING FIX */}
            {showHistoryId === app.id && (
              <div className="absolute inset-0 bg-white/98 backdrop-blur-md z-50 p-10 flex flex-col rounded-[2.8rem] animate-fade-in shadow-2xl">
                <div className="flex justify-between items-center mb-8">
                  <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest font-black">Timeline Log (SGT)</span>
                ...
                <div className="flex-grow overflow-y-auto space-y-8 pr-2">
                  {app.status_history?.map((h, i) => (
                    <div key={i} className="relative pl-8 border-l-2 border-blue-500 py-1">
                      <div className="absolute -left-[5px] top-1 w-2 h-2 rounded-full bg-blue-600 shadow-lg shadow-blue-200"></div>
                      <div className="text-xs font-black uppercase text-slate-800 tracking-tight">{h.status}</div>
                      <div className="text-[10px] text-slate-500 font-bold mt-1">
                        {/* THE CRITICAL DISPLAY FIX */}
                        {formatSGT(h.date)}
                      </div>
                    </div>
                  ))}
