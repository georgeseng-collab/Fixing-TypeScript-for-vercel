...
export default function Dashboard() {
  ...
  const openGoogleCalendar = (app, type = 'INTERVIEW') => {
    // 1. Get Date
    const dateInput = window.prompt(`Select Date for ${type} (YYYY-MM-DD):`, new Date().toLocaleDateString('en-CA'));
    if (!dateInput) return;

    // 2. Get Time
    const timeInput = window.prompt(`Select Start Time (24h format HH:MM):`, "10:00");
    if (!timeInput) return;
    ...
    const endTime = `${cleanDate}T${endH}${m.toString().padStart(2, '0')}00`;

    const title = encodeURIComponent(`${type}: ${app.name} (${app.job_role})`);
    const details = encodeURIComponent(`Candidate: ${app.name}\nRole: ${app.job_role}\nEmail: ${app.email}\nPhone: ${app.phone}`);
    
    // CRITICAL: Added &ctz=Asia/Singapore to lock the timezone
  ...
  const handleStatusChange = async (app, newStatus) => {
    ...
    if (newStatus === 'Interviewing') {
      if (window.confirm("Schedule this interview in Google Calendar?")) {
        openGoogleCalendar(app, 'INTERVIEW');
      }
    }

    if (newStatus === 'Hired') {
      if (window.confirm("Schedule onboarding in Google Calendar?")) {
        openGoogleCalendar(app, 'ONBOARDING');
      }
    }
  ...
  if (loading) return <div className="flex h-screen items-center justify-center font-black text-slate-300 animate-pulse uppercase tracking-widest">Geniebook Talent Sync...</div>;
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 pb-24 space-y-10">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-white p-6 rounded-[2.5rem] border border-slate-50 shadow-sm">
        <h1 className="text-4xl font-black text-slate-900 tracking-tighter">Hiring Pipeline</h1>
        <input 
          type="text" 
          placeholder="Search candidates..." 
          className="bg-slate-50 px-6 py-3 rounded-2xl outline-none border border-transparent focus:border-blue-500 w-full md:w-80 font-bold text-sm"
          ...
          <div key={app.id} className="bg-white rounded-[3rem] border border-slate-100 shadow-xl shadow-slate-200/40 flex flex-col overflow-hidden transition-all hover:shadow-2xl">
            ...
            <div className={`p-8 pb-4 border-t-[14px] ${getStatusTheme(app.status).split(' ')[0]}`}>
              <div className="flex justify-between items-start">
                ...
                <button onClick={() => editId === app.id ? saveEdit() : (setEditId(app.id), setEditData(app))} className="p-3 bg-slate-50 rounded-2xl hover:bg-slate-900 hover:text-white transition-all">
                  {editId === app.id ? '💾' : '✏️'}
                </button>
              </div>
            </div>
            ...
            <div className="p-8 pt-0 space-y-6 flex-grow">
              ...
              <div className="grid grid-cols-2 gap-2">
                <a href={`mailto:${app.email}`} className="flex items-center justify-center bg-slate-50 p-3 rounded-2xl text-[10px] font-black text-slate-500 hover:bg-slate-100">EMAIL</a>
              ...
              <button 
                ...
                className="w-full flex items-center justify-center gap-2 py-4 bg-amber-50 border border-amber-100 rounded-3xl text-amber-700 font-black text-[11px] uppercase tracking-widest hover:bg-amber-100 transition-all"
              >
                📅 Schedule Interview (SGT)
              </button>
            ...
            {showHistoryId === app.id && (
              <div className="absolute inset-0 bg-white z-50 p-10 flex flex-col rounded-[2.8rem] shadow-2xl animate-fade-in">
                <div className="flex justify-between items-center mb-10 border-b pb-4">
                  <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Status Log</span>
