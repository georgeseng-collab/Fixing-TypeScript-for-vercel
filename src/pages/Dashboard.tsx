...
export default function Dashboard() {
  ...
  const saveEdit = async () => {
    // This saves EVERY field in the editData object back to Supabase
  ...
  const inputEditClass = "w-full bg-white/20 rounded-lg px-3 py-1 outline-none text-white font-bold border border-white/30 mb-1";
          ...
          <div key={app.id} className="bg-white rounded-[4rem] border-4 border-slate-900 shadow-[10px_10px_0px_0px_rgba(15,23,42,1)] overflow-hidden flex flex-col transition-all hover:translate-y-[-8px]">
            ...
            <div className={`p-10 pb-6 ${getStatusTheme(app.status)} border-b-4 border-slate-900`}>
              <div className="flex items-center justify-between gap-4">
                <div className="flex-grow">
                  {editId === app.id ? (
                    <div className="space-y-1">
                      <input className={inputEditClass} value={editData.name} onChange={e => setEditData({...editData, name: e.target.value})} placeholder="Name" />
                      <input className={inputEditClass + " text-[10px] uppercase"} value={editData.job_role} onChange={e => setEditData({...editData, job_role: e.target.value})} placeholder="Job Role" />
                    </div>
                  ) : (
                    <div className="space-y-1">
                      {/* FIXED: break-words and leading-tight for long names */}
                ...
                <button onClick={() => editId === app.id ? saveEdit() : (setEditId(app.id), setEditData(app))} className="w-14 h-14 flex items-center justify-center bg-white/20 rounded-2xl hover:bg-white/40 shrink-0 border-2 border-white/20">
                  {editId === app.id ? '✔️' : '✏️'}
                </button>
              </div>
            </div>
            ...
            <div className="p-10 space-y-8 flex-grow">
              <div className="space-y-3">
                {editId === app.id ? (
                  ...
                  <>
                    <a href={`mailto:${app.email}`} className="flex items-center gap-4 bg-slate-50 p-5 rounded-[1.5rem] text-[11px] font-black text-slate-600 border-2 border-slate-900 shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] uppercase">
                      <span>📧</span> {app.email}
                    </a>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-slate-50 p-5 rounded-[1.5rem] border-2 border-slate-900 shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] text-center">
                  <span className="block text-[8px] font-black text-slate-400 uppercase mb-1 underline decoration-slate-200">Last Drawn</span>
                  {editId === app.id ? (
                    <input className="w-full text-center bg-transparent font-black outline-none border-b border-slate-400" value={editData.current_salary} onChange={e => setEditData({...editData, current_salary: e.target.value})} />
                  ) : (
                    <span className="text-sm font-black">${app.current_salary || app.last_drawn_salary || '0'}</span>
                  )}
                </div>
                <div className="bg-slate-50 p-5 rounded-[1.5rem] border-2 border-slate-900 shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] text-center">
                  <span className="block text-[8px] font-black text-slate-400 uppercase mb-1 underline decoration-slate-200">Expected</span>
                  {editId === app.id ? (
                    <input className="w-full text-center bg-transparent font-black outline-none border-b border-slate-400" value={editData.expected_salary} onChange={e => setEditData({...editData, expected_salary: e.target.value})} />
                  ) : (
              ...
              <div className="space-y-4">
                <select value={app.status} onChange={e => handleStatusChange(app.id, e.target.value)} className={`w-full py-5 rounded-[2rem] text-[11px] font-black uppercase border-4 border-slate-900 shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] outline-none text-center appearance-none cursor-pointer ${getStatusTheme(app.status)}`}>
              ...
              {showHistoryId === app.id && (
                <div className="mt-4 p-6 bg-slate-900 rounded-[2.5rem] text-white space-y-4 animate-in fade-in zoom-in duration-300 shadow-[6px_6px_0px_0px_rgba(0,0,0,1)]">
                     ...
                     {app.status_history?.map((h, i) => (
                       <div key={i} className="border-l-2 border-white/20 pl-4 py-1">
                          <div className="flex justify-between items-center mb-1">
                            <span className="text-[10px] font-black uppercase tracking-tighter">{h.status}</span>
                            <span className="text-[8px] font-bold text-white/40">{new Date(h.date).toLocaleDateString()}</span>
                          </div>
                          {h.salary && <div className="text-[9px] font-black text-emerald-400 italic">Offer: ${h.salary}</div>}
                          {h.remarks && <div className="text-[9px] font-bold text-white/60 italic mt-1 leading-tight">"{h.remarks}"</div>}
                       </div>
                     ))}
                   </div>
                </div>
