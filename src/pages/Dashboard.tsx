          ...
          return (
            <div key={app.id} className="bg-white rounded-[4rem] border-4 border-slate-900 shadow-[10px_10px_0px_0px_rgba(15,23,42,1)] overflow-hidden flex flex-col">
              ...
              <div className={`p-10 pb-6 ${getStatusTheme(app.status)} border-b-4 border-slate-900`}>
                ...
                <p className="text-[10px] font-black uppercase opacity-70 mt-1">{isArchivedView ? `Status: ${app.status}` : app.job_role}</p>
              </div>
              ...
              <div className="p-10 space-y-6 flex-grow">
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex items-center gap-4 bg-emerald-50 p-5 rounded-[1.5rem] text-[11px] font-black text-emerald-700 border-2 border-slate-900 shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] uppercase">
                    <span>📱</span> {app.phone}
                  </div>
                </div>

                {/* ARCHIVE SPECIFIC VIEW */}
                {isArchivedView ? (
                  <div className="bg-slate-900 p-8 rounded-[3rem] space-y-4 text-white">
                    {app.status === 'Resigned' && (
                      <div className="space-y-1">
                        <span className="text-[9px] font-black text-blue-400 uppercase tracking-widest">Date of Leaving</span>
                        <p className="text-sm font-bold">{app.resignation_date || 'Not Recorded'}</p>
                      </div>
                    )}
                    <div className="space-y-1">
                      <span className="text-[9px] font-black text-blue-400 uppercase tracking-widest">Reason / Remarks</span>
                  ...
                  <div className="space-y-6">
                    ...
                    {app.status === 'Offer Accepted' && isFullyReady ? (
                      <button onClick={() => updateStatus(app.id, 'Hired', 'Onboarded via Dashboard', '')} className="w-full py-6 bg-emerald-500 text-white rounded-[2rem] border-4 border-slate-900 font-black uppercase shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] animate-pulse">🎉 Onboard Now</button>
                    ) : (
