    ...
    <div className="max-w-7xl mx-auto px-6 py-12 space-y-8 pb-32 relative">
      ...
      <div className="flex flex-wrap gap-3">
        {Object.entries(stats).map(([label, count]) => (
          <button key={label} onClick={() => setFilterStatus(label)} className={`flex-1 min-w-[140px] p-6 rounded-[2rem] border-4 border-slate-900 shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] transition-all ${filterStatus === label ? 'bg-slate-900 text-white' : 'bg-white text-slate-900'}`}>
        ...
        {filtered.map(app => {
          ...
          return (
            <div key={app.id} className="bg-white rounded-[4rem] border-4 border-slate-900 shadow-[14px_14px_0px_0px_rgba(15,23,42,1)] overflow-hidden flex flex-col transition-all">
              ...
              {/* Card Header (EDIT BUTTON RESTORED) */}
              <div className={`p-10 pb-8 ${getStatusTheme(app.status)} border-b-4 border-slate-900`}>
                <div className="flex items-center justify-between gap-4">
                  ...
                  <button onClick={() => editId === app.id ? saveEdit() : (setEditId(app.id), setEditData(app))} className="w-14 h-14 flex items-center justify-center bg-white/20 rounded-2xl border-2 border-white/20 shrink-0 hover:bg-white/40">
                    {editId === app.id ? '✔️' : '✏️'}
                  </button>
                </div>
              </div>
              ...
              <div className="p-10 space-y-8 flex-grow">
                ...
                {isArchivedView ? (
                  <div className="bg-slate-900 p-8 rounded-[3.5rem] space-y-5 text-white">
                    ...
                    <div className="space-y-1"><span className="text-[9px] font-black text-blue-400 uppercase italic">Reason / Remarks</span><p className="text-xs italic opacity-80 leading-relaxed font-medium">"{app.remarks || 'No remarks provided.'}"</p></div>
                  </div>
                ) : (
                    ...
                    {(app.status === 'Offered' || app.status === 'Offer Accepted') && (
                      <div className="bg-slate-50 p-6 rounded-[2.5rem] border-2 border-slate-900 space-y-4 shadow-inner">
                        <div className="flex justify-between text-[11px] font-black uppercase italic">
                           <span className="text-slate-400 underline italic">1. Offer Hub</span>
                        ...
                        <div className="flex justify-between text-[11px] font-black uppercase italic">
                           <span className="text-slate-400 underline italic">2. Approval Hub</span>
                ...
                {/* SALARY INFO (EDITABLE IN EDIT MODE) */}
