...
export default function EmailHub() {
  ...
  const [isFullTimeStaff, setIsFullTimeStaff] = useState(true); // Toggle for the Benefit block
  const [isSingaporean, setIsSingaporean] = useState(true); // Toggle for Training Cost ($2k vs $1k)
  ...
  const schedules = {
    ...
    "Curriculum (5+1)": { days: "5 Weekdays + 1 Weekend", hours: "• Weekdays : 12pm to 9pm\n• Weekends : 9am to 6.30pm" },
    "Curriculum (3+2)": { days: "3 Weekdays + 2 Weekend", hours: "• Weekdays : 12pm to 9pm\n• Weekends : 9am to 6.30pm" },
    "Curriculum (4+1)": { days: "4 Weekdays + 1 Weekend", hours: "• Weekdays : 12pm to 9pm\n• Weekends : 9am to 6.30pm" },
  ...
  const trainingCost = isSingaporean ? "$2,000" : "$1,000";
  ...
  const copyToClipboard = () => {
    ...
    window.getSelection().removeAllRanges();
    alert("Email Content Copied! You can now Paste (Ctrl+V) into Gmail.");
  };
    ...
    <div className="max-w-6xl mx-auto px-6 py-10 pb-40">
      <div className="flex justify-between items-end mb-10 border-b-4 border-slate-900 pb-6">
        <div>
          ...
          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1 italic">GenieBook Internal Tool</p>
        </div>
        ...
        <div className="flex gap-4">
           ...
           <div className="bg-slate-100 p-1 rounded-xl flex gap-1">
              <button onClick={() => setIsSingaporean(true)} className={`px-4 py-2 rounded-lg text-[9px] font-black uppercase ${isSingaporean ? 'bg-white shadow text-emerald-600' : 'text-slate-400'}`}>Singaporean ($2k)</button>
              <button onClick={() => setIsSingaporean(false)} className={`px-4 py-2 rounded-lg text-[9px] font-black uppercase ${!isSingaporean ? 'bg-white shadow text-emerald-600' : 'text-slate-400'}`}>Malaysian ($1k)</button>
           </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
        ...
        <div className="lg:col-span-4 bg-white p-8 rounded-[3rem] shadow-xl space-y-4 h-fit border border-slate-100">
          <div>
            <label className="text-[9px] font-black uppercase text-slate-400 ml-2 tracking-widest">Select Candidate</label>
        ...
        <div className="lg:col-span-8 bg-white p-12 rounded-[4rem] shadow-2xl border border-slate-100">
          <div id="email-content" style={{ color: '#334155', fontFamily: 'Arial, sans-serif', fontSize: '14px', lineHeight: '1.6' }}>
            ...
            <p>... pay for training costs of <strong>{trainingCost}</strong> if you resign within 3 months...</p>
            <br />
