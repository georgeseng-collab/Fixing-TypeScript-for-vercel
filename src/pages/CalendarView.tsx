...
export default function CalendarView() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // --- RESUME PROCESSING: FETCH FROM STORAGE ---
      let base64Resume = "";
      ...
      {showSuccessModal && (
        <div className="fixed inset-0 bg-black/80 z-[200] flex items-center justify-center p-6 backdrop-blur-md">
            <div className="bg-white border-8 border-black p-12 rounded-[3rem] max-w-md w-full text-center shadow-[20px_20px_0_0_#000]">
                ...
                <p className="font-bold text-slate-500 mb-8 uppercase text-xs tracking-widest leading-tight text-center">Calendar invite sent & Resume linked to Google Drive</p>
                ...
                <div className="space-y-6">
                  ...
                  {selectedApp && (
                    <div className={`p-4 border-2 border-black rounded-xl font-black text-xs uppercase italic flex items-center gap-3 ${selectedApp.resume_metadata?.url ? 'bg-emerald-50 text-emerald-700' : 'bg-rose-50 text-rose-700'}`}>
                        <span>{selectedApp.resume_metadata?.url ? '✓ Resume storage link verified' : '⚠️ No Resume found in Supabase storage'}</span>
                    </div>
                  )}
                ...
                <div className="space-y-6">
                  ...
                  {hasScanned && busyPeople.length > 0 && (
                    <div className="p-5 bg-amber-100 border-4 border-amber-500 rounded-2xl shadow-[6px_6px_0_0_#f59e0b]">
                        ...
                        <p className="text-[10px] font-bold mt-1 text-amber-900 opacity-80 uppercase">
                            Busy: {busyPeople.map(email => teamMembers.find(t => t.email === email)?.name || email).join(", ")}
                        </p>
                    </div>
                  )}
                  ...
                  {hasScanned && roomAlt.length > 0 && selectedRoom && !suggestions.includes(formTime) && (
                    <div className="p-5 bg-rose-600 border-4 border-black shadow-[6px_6px_0_0_#000] text-white animate-pulse rounded-2xl">
                      <p className="font-black uppercase text-xs">⚠️ Room Conflict! Use these instead:</p>
                      <div className="flex flex-wrap gap-2 mt-2">
                        {roomAlt.map(alt => (
                          <button key={alt.email} onClick={() => { setSelectedRoom(alt.email); handleManualScan(); }} className="bg-white text-rose-600 px-4 py-1.5 rounded-lg border-2 border-black font-black text-[10px] hover:bg-yellow-300">Use {alt.name}</button>
                        ))}
                      </div>
                    </div>
                  )}
                  ...
                  <div className="grid grid-cols-2 gap-6">
                    <div className="space-y-4">
                      <div className="p-4 bg-yellow-300 border-4 border-black rounded-2xl shadow-[4px_4px_0_0_#000]">
                        ...
                        <input type="date" value={formDate} onChange={e => setFormDate(e.target.value)} className="w-full p-2 border-2 border-black font-black mt-1 rounded-xl" />
                      </div>
                      ...
                      <div className="p-4 bg-white border-4 border-black rounded-2xl shadow-[4px_4px_0_0_#000]">
                        ...
                        <select className="w-full p-2 border-2 border-black font-black mt-1 outline-none rounded-xl" value={selectedRoom} onChange={e => setSelectedRoom(e.target.value)}>
                    ...
                    <div className="space-y-2">
                      <label className="font-black text-[10px] uppercase italic ml-2 text-black">Internal Panel</label>
                          ...
                          {teamMembers.map(m => {
                            ...
                            return (
                              <button key={m.email} onClick={() => setSelectedGuests(prev => prev.includes(m.email) ? prev.filter(x => x !== m.email) : [...prev, m.email])}
                                  className={`p-2 border-2 border-black font-black text-[10px] uppercase transition-all rounded-xl ${selectedGuests.includes(m.email) ? 'bg-blue-600 text-white' : 'bg-white shadow-[2px_2px_0_0_#000]'} ${isUnreachable ? 'opacity-40 italic' : ''}`}>
                                  {m.name}
                              </button>
                            );
                          })}
                  ...
                  <div className={`p-6 border-4 border-black bg-blue-50 shadow-[8px_8px_0_0_#000] min-h-[200px] rounded-[2rem]`}>
                    {!hasScanned ? (
                      ...
                      <div className="flex flex-col items-center justify-center">
                        ...
                        <p className="font-black text-xs animate-pulse text-black uppercase italic">Pinging Google...</p>
                      </div>
                    ) : (
                      <div className="space-y-4">
                        <p className="font-black text-[10px] uppercase italic text-black">Free Gaps Found ({duration}m):</p>
                  ...
                  <div className="p-4 border-4 border-black bg-slate-100 flex items-center justify-between rounded-2xl">
                    <label className="flex items-center gap-3 cursor-pointer group">
                        ...
                        <span className="font-black text-xs uppercase italic text-black group-hover:text-blue-600">Manual Force (Bypass Gaps)</span>
                    </label>
                    {bypassConflict && (
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <button disabled={!formTime} onClick={() => setStep(3)} className="p-4 bg-black text-white font-black uppercase shadow-[6px_6px_0_0_#000] active:translate-y-1 transition-all rounded-2xl">Review →</button>
                  </div>
                </div>
              )}
              ...
              {step === 3 && (
                <div className="space-y-6">
                  ...
                  <div className="bg-slate-900 text-white p-10 border-4 border-black rounded-[2.5rem] shadow-[12px_12px_0_0_#000] space-y-8 relative overflow-hidden">
                      ...
                      <div className="relative z-10">
                        <div className="border-b border-white/20 pb-4 mb-6">
                            ...
                            <h3 className="font-black text-5xl uppercase tracking-tighter text-white">{selectedApp?.name}</h3>
                        </div>
                        <div className="grid grid-cols-2 gap-10">
                            <div>
                                <p className="text-[10px] uppercase font-black opacity-40 text-white">Schedule</p>
                                <p className="font-black text-2xl italic text-white">{formDate}</p>
                                <p className="font-black text-4xl text-emerald-400 mt-2 uppercase">{formTime ? format(parse(formTime, 'HH:mm', new Date()), 'hh:mm a') : 'MANUAL'}</p>
                            </div>
                            <div>
                                <p className="text-[10px] uppercase font-black opacity-40 text-white">Venue</p>
                                <p className="font-black text-xl leading-tight uppercase text-white underline decoration-emerald-500">{MEETING_ROOMS.find(r => r.email === selectedRoom)?.name || 'Online'}</p>
                            </div>
                        </div>
                      </div>
                  </div>
                  ...
                  <div className="space-y-1">
                    ...
                    <input type="text" value={customGuest} onChange={e => setCustomGuest(e.target.value)} className="w-full p-5 border-4 border-black font-black outline-none bg-white rounded-2xl shadow-inner text-lg text-black" placeholder="CC Janice etc." />
                  </div>
