...
export default function CalendarView() {
    ...
    try {
      ...
      const calendarEvents = (data || []).flatMap(app => {
        ...
        return history
          ...
          .map((h, idx) => ({
            // FIX: Using a unique delimiter that isn't in a UUID
            id: `${app.id}___${idx}`, 
            candidateId: app.id,
            candidate: app,
            historyIndex: idx,
  ...
  // --- REBUILT DELETE LOGIC (UUID SAFE) ---
  const confirmDelete = async () => {
    if (!selectedEventId || !selectedApp) return;

    // Use the triple-underscore to separate ID from index safely
    const candId = selectedEventId.split('___')[0]; 
    const idx = selectedHistoryIndex;

    setShowModal(false);
    setEvents(prev => prev.filter(e => e.id !== selectedEventId));

    try {
      // 1. Fetch fresh data to ensure array integrity
      ...
      // 2. Splice the history
      const newHistory = [...(fresh.status_history || [])];
      newHistory.splice(idx, 1);

      // 3. Update Supabase with the FULL UUID
      ...
      console.log("Delete success for UUID:", candId);
      fetchData();
    } catch (err) {
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 py-10 relative">
      <div className="bg-white p-10 rounded-[4rem] shadow-xl border border-slate-50 flex justify-between items-center mb-10">
        ...
        <button onClick={() => { setIsDeleting(false); setSelectedApp(null); setStep(1); setShowModal(true); }} className="bg-blue-600 text-white px-10 py-5 rounded-[2rem] font-black text-xs uppercase shadow-2xl hover:bg-blue-700 active:scale-95 transition-all tracking-widest">+ New Schedule</button>
      </div>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-md z-[100] flex items-center justify-center p-6">
          <div className="bg-white w-full max-w-lg rounded-[4rem] shadow-2xl overflow-hidden animate-in zoom-in duration-100 border border-white">
            <div className={`p-10 text-white flex justify-between items-center ${isDeleting ? 'bg-rose-600' : 'bg-slate-900'}`}>
              <h3 className="text-2xl font-black italic tracking-tight">{isDeleting ? 'Delete Schedule?' : 'Arrange Interview'}</h3>
            ...
            <div className="p-10">
              {isDeleting ? (
                <div className="space-y-6 text-center">
                  <p className="font-bold text-slate-600 italic leading-relaxed">Remove interview for <br/><span className="text-rose-600 text-2xl font-black underline decoration-rose-200 decoration-4 underline-offset-8">{selectedApp?.name}</span>?</p>
                  <div className="flex gap-4 pt-6">
                    <button onClick={() => setShowModal(false)} className="flex-1 py-4 bg-slate-50 text-slate-400 rounded-2xl font-black text-[10px] uppercase tracking-widest">Cancel</button>
                    <button onClick={confirmDelete} className="flex-1 py-4 bg-rose-600 text-white rounded-2xl font-black text-[10px] uppercase shadow-lg active:scale-95 transition-all tracking-widest">Confirm Delete</button>
                  </div>
                </div>
              ) : step === 1 ? (
                <div className="space-y-4">
                  <div className="space-y-2 max-h-[350px] overflow-y-auto no-scrollbar pr-1">
                    {applicants.filter(a => !['Blacklisted', 'Failed Interview', 'Resigned'].includes(a.status)).map(app => (
                      <button key={app.id} onClick={() => { setSelectedApp(app); setStep(2); }} className="w-full text-left p-6 hover:bg-blue-50 rounded-[2rem] border border-slate-50 flex items-center justify-between group transition-all">
                        ...
                        <span className="text-blue-500 font-black opacity-0 group-hover:opacity-100 transition-all translate-x-[-10px] group-hover:translate-x-0">SELECT →</span>
                      </button>
                    ))}
                  </div>
                </div>
                ...
                <div className="space-y-8 animate-in slide-in-from-right-4 duration-300">
                  <div className="bg-slate-50 p-6 rounded-[2.5rem] border border-slate-100">
                    <div className="text-xl font-black text-slate-800 tracking-tight">{selectedApp?.name}</div>
                    <div className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{selectedApp?.job_role}</div>
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-1">
                      <label className="text-[9px] font-black uppercase text-slate-400 ml-4 tracking-widest">Date</label>
                    ...
                    <div className="space-y-1">
                      <label className="text-[9px] font-black uppercase text-slate-400 ml-4 tracking-widest">Time</label>
