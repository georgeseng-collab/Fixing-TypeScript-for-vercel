...
export default function CalendarView() {
  ...
  const [selectedRoom, setSelectedRoom] = useState(''); // <--- Added Room State
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // 1. Sync to Google (Includes Room clash check and file renaming logic)
  ...
  const confirmDelete = async () => {
    if (!activeEventDate || !window.confirm("DELETE INTERVIEW? This removes it from ATS record.")) return;
    try {
      setIsSyncing(true);
    ...
    <div className="max-w-[1700px] mx-auto px-8 py-10 pb-32">
      <div className="bg-white dark:bg-slate-900 p-10 rounded-[3rem] border-4 border-slate-900 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] flex justify-between items-center mb-10 transition-colors">
        ...
        <button 
          ...
          className="bg-blue-600 text-white px-10 py-5 rounded-2xl border-4 border-slate-900 font-black text-xs uppercase shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] hover:bg-slate-900 transition-all active:scale-95"
        >
          + Create Event
        </button>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 p-8 rounded-[3rem] border-4 border-slate-900 shadow-[15px_15px_0px_0px_rgba(0,0,0,1)] h-[850px] transition-colors overflow-hidden">
        <Calendar 
          ...
          eventPropGetter={() => ({ 
            style: { backgroundColor: '#2563eb', borderRadius: '15px', border: '3px solid #0f172a', fontWeight: '900', fontSize: '11px' } 
          })}
        />
      </div>
              ...
              {step === 1 ? (
                    ...
                    {applicants
                      ...
                      .map(app => (
                        <button key={app.id} onClick={() => { setSelectedApp(app); setStep(2); }} className="w-full text-left p-6 bg-white dark:bg-slate-800 hover:bg-blue-600 hover:text-white rounded-3xl border-4 border-slate-900 flex justify-between items-center transition-all group active:scale-95 shadow-md">
                          <span className="font-black text-xl uppercase italic">{app.name}</span>
                ...
                <div className="space-y-8">
                  ...
                  <div className="grid grid-cols-2 gap-6">
                    <input type="date" className="p-5 border-4 border-slate-900 rounded-[2rem] font-black text-sm outline-none dark:bg-slate-800" value={formDate} onChange={e => setFormDate(e.target.value)} />
                    <input type="time" className="p-5 border-4 border-slate-900 rounded-[2rem] font-black text-sm outline-none dark:bg-slate-800" value={formTime} onChange={e => setFormTime(e.target.value)} />
                  </div>

                  <div className="space-y-2">
                    <label className="text-[11px] font-black uppercase text-slate-400 ml-4 italic leading-none">Book Room Slot</label>
                    <select 
                      className="w-full p-5 border-4 border-slate-900 rounded-[2rem] font-black text-sm outline-none bg-white dark:bg-slate-800"
                      value={selectedRoom}
                  ...
                  <div className="flex flex-col gap-4 pt-10 border-t-8 border-slate-100 dark:border-slate-800">
                    ...
                    >
                      {isSyncing ? 'Checking Availability...' : 'Confirm Schedule & Notify'}
                    </button>
                    {isManagementMode && (
