...
export default function CalendarView() {
  ...
  const handleSelectCandidate = async (appId) => {
    ...
    // SOURCE 1: Try getting from 'applicants' table directly
    let { data: fullApp } = await supabase.from('applicants').select('*').eq('id', appId).single();

    // SOURCE 2: If no resume string in 'applicants', check a potential 'resumes' table
    if (fullApp && !fullApp.resume_base64 && !fullApp.resume) {
        const { data: resumeEntry } = await supabase.from('resumes').select('resume_base64, resume').eq('applicant_id', appId).single();
        if (resumeEntry) {
            fullApp.resume_base64 = resumeEntry.resume_base64 || resumeEntry.resume;
        }
    }

    setSelectedApp(fullApp || baseCandidate);
  };
      ...
      {showSuccessModal && (
        <div className="fixed inset-0 bg-black/80 z-[200] flex items-center justify-center p-6 backdrop-blur-md">
            <div className="bg-white border-8 border-black p-12 rounded-[3rem] max-w-md w-full text-center shadow-[20px_20px_0_0_#000]">
                ...
                <button onClick={() => { setShowSuccessModal(false); resetForm(); }} className="w-full bg-black text-white p-4 rounded-xl font-black uppercase hover:bg-blue-600 transition-all">Done</button>
            </div>
        </div>
      )}
          ...
          <div className="bg-white border-8 border-black w-full max-w-4xl max-h-[95vh] overflow-y-auto p-10 rounded-[3rem] shadow-[25px_25px_0_0_#000]">
            ...
            {step === 1 && (
              <div className="space-y-6">
                <h2 className="text-4xl font-black italic uppercase text-black">1. Select Candidate</h2>
                ...
                {selectedApp && (
                  <div className={`p-4 border-2 border-black rounded-xl font-black text-xs uppercase italic ${ (selectedApp.resume_base64 || selectedApp.resume) ? 'bg-emerald-50 text-emerald-700 border-emerald-500' : 'bg-rose-50 text-rose-700 border-rose-500'}`}>
                      <span>{ (selectedApp.resume_base64 || selectedApp.resume) ? '✓ Resume string found' : '⚠️ Missing Resume in applicants/resumes tables'}</span>
                  </div>
                )}
                <button disabled={!selectedApp} onClick={() => setStep(2)} className="w-full p-6 bg-black text-white font-black uppercase shadow-[8px_8px_0_0_#000] hover:bg-blue-600 transition-all rounded-2xl">Logistics →</button>
              </div>
            )}

            {step === 2 && (
              <div className="space-y-6">
                <h2 className="text-4xl font-black italic uppercase tracking-tighter leading-none text-black">2. Scanning</h2>
                
                <div className="grid grid-cols-2 gap-6">
                    <div className="space-y-4">
                        <div className="p-4 bg-yellow-300 border-4 border-black rounded-2xl shadow-[4px_4px_0_0_#000]">
                          <label className="font-black text-[10px] uppercase italic">Date</label>
                        ...
                        <div className="p-4 bg-white border-4 border-black rounded-2xl">
                          <label className="font-black text-[10px] uppercase italic text-black">Room</label>
                          <select className="w-full p-2 border-2 border-black font-black mt-1 text-black" value={selectedRoom} onChange={e => setSelectedRoom(e.target.value)}>
                    ...
                    <div className="p-4 bg-slate-50 border-4 border-black h-[220px] overflow-y-auto rounded-2xl">
                      <label className="font-black text-[10px] uppercase italic text-black">Panel</label>
                      <div className="space-y-2 mt-2">
                        {teamMembers.map(m => (
                          <button key={m.email} onClick={() => setSelectedGuests(prev => prev.includes(m.email) ? prev.filter(x => x !== m.email) : [...prev, m.email])}
                              className={`w-full p-2 border-2 border-black font-black text-[10px] uppercase rounded-xl ${selectedGuests.includes(m.email) ? 'bg-blue-600 text-white' : 'bg-white shadow-[2px_2px_0_0_#000] text-black'}`}>
                              {m.name}
                          </button>
                        ))}
                      </div>
                ...
                <div className="p-6 border-4 border-black bg-blue-50 rounded-[2rem]">
                  {!hasScanned ? (
                    <button onClick={handleManualScan} className="w-full bg-blue-600 text-white font-black p-5 border-4 border-black shadow-[4px_4px_0_0_#000] uppercase rounded-2xl">Scan Free Slots</button>
                  ) : isScanning ? (
                    <p className="font-black text-xs animate-pulse text-center text-black">Syncing...</p>
                  ) : (
                    <div className="grid grid-cols-5 gap-2 text-black">
                      {suggestions.map(t => ( <button key={t} onClick={() => {setFormTime(t); setBypassConflict(false);}} className={`p-2 border-2 border-black font-black text-[10px] rounded-xl transition-all ${formTime === t ? 'bg-emerald-500 text-white' : 'bg-white shadow-[3px_3px_0_0_#000] text-black'}`}>{format(parse(t, 'HH:mm', new Date()), 'hh:mm a')}</button> ))}
                    </div>
                  )}
                </div>
            ...
            {step === 3 && (
              <div className="space-y-6">
                <h2 className="text-4xl font-black italic uppercase tracking-tighter text-black">3. Final Review</h2>
                <div className="bg-slate-900 text-white p-10 border-4 border-black rounded-[2.5rem] shadow-[12px_12px_0_0_#000]">
                    <h3 className="font-black text-5xl uppercase mb-6 text-white">{selectedApp?.name}</h3>
                    <div className="grid grid-cols-2 gap-10">
                        <div><p className="text-[10px] uppercase opacity-40 text-white">Schedule</p><p className="font-black text-2xl text-white">{formDate}</p><p className="font-black text-4xl text-emerald-400">{formTime}</p></div>
                        <div><p className="text-[10px] uppercase opacity-40 text-white">Venue</p><p className="font-black text-xl uppercase text-white">{MEETING_ROOMS.find(r => r.email === selectedRoom)?.name || 'Online'}</p></div>
                    </div>
                </div>
                <input type="text" value={customGuest} onChange={e => setCustomGuest(e.target.value)} className="w-full p-5 border-4 border-black font-black bg-white rounded-2xl text-black" placeholder="CC Janice etc." />
                <div className="grid grid-cols-2 gap-4">
                  <button onClick={() => setStep(2)} className="p-5 border-4 border-black font-black rounded-2xl text-black">← Back</button>
