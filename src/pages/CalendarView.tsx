...
export default function CalendarView() {
  ...
  const handleSelectEvent = (event) => {
    ...
    setSelectedEventId(event.id); // Format: "uuid-index"
    setIsDeleting(true);
    setShowModal(true);
  };
  ...
  const confirmDelete = async () => {
    ...
    // We remove it from the local state IMMEDIATELY so it vanishes from the screen
    ...
    if (error) {
      ...
      alert("Failed to sync with database. Refreshing...");
      fetchData(); // If it failed, bring the event back
    } else {
      // Subtle background refresh to ensure all indices are correct for next time
      fetchData(); 
    }
  };
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 py-10 relative">
      <div className="bg-white p-10 rounded-[3rem] shadow-xl border border-slate-50 flex justify-between items-center mb-10">
      ...
      {showModal && (
        <div className="fixed inset-0 bg-slate-900/70 backdrop-blur-md z-[100] flex items-center justify-center p-6">
          <div className="bg-white w-full max-w-md rounded-[4rem] shadow-2xl overflow-hidden animate-in zoom-in duration-100">
            ...
            <div className="p-10">
              {isDeleting ? (
                <div className="space-y-6 text-center">
                  <p className="font-bold text-slate-600 leading-relaxed">Remove interview for <br/><span className="text-rose-600 text-xl font-black underline">{selectedApp?.name}</span>?</p>
                ...
                <div className="space-y-4">
                  <input type="text" placeholder="Search candidate..." className="w-full p-4 bg-slate-50 rounded-2xl font-bold text-sm mb-4 outline-none border-2 border-transparent focus:border-blue-500 transition-all" value={searchCandidate} onChange={(e) => setSearchCandidate(e.target.value)} />
                  <div className="space-y-2 max-h-[300px] overflow-y-auto no-scrollbar">
                    {applicants.filter(a => !['Blacklisted', 'Failed'].includes(a.status) && a.name.toLowerCase().includes(searchCandidate.toLowerCase())).map(app => (
                      <button key={app.id} onClick={() => { setSelectedApp(app); setStep(2); }} className="w-full text-left p-5 hover:bg-blue-50 rounded-[1.5rem] border border-slate-50 flex items-center justify-between group transition-all">
                        <div><div className="font-black text-slate-800 text-lg group-hover:text-blue-600">{app.name}</div><div className="text-[10px] font-bold text-slate-400 uppercase">{app.job_role}</div></div>
